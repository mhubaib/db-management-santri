    ...
    <div class="bg-white overflow-hidden shadow-xl rounded-lg border border-gray-100">
        <div class="px-8 py-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-primary-800">Daftar Ustadz</h2>
            ...
            <div class="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-100">
                <form action="{{ route('ustadz.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                    ...
                    <div class="flex gap-3">
                        <select name="spesialisasi"
                            ...
                            @foreach ($spesialisasis as $s)
                                <option value="{{ $s }}" {{ request('spesialisasi') == $s ? 'selected' : '' }}>
                                    {{ $s }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        @if (request()->anyFilled(['search', 'spesialisasi']))
                    ...
                    <thead>
                        <tr class="bg-gray-50">
                            <th scope="col"
                                ...
                                No</th>
                            <th scope="col"
                                class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">NIP
                            </th>
                            <th scope="col"
                        ...
                        @forelse ($ustadzs as $index => $ustadz)
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ ($ustadzs->currentPage() - 1) * $ustadzs->perPage() + $loop->iteration }}
                                </td>
                            ...
                            <tr>
                                <td colspan="6"
                                    ...
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="font-medium">Tidak ada data ustadz</p>
                                        <p class="text-gray-400 mt-1">Silahkan tambahkan data ustadz terlebih dahulu</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const filterSpesialisasi = document.querySelector('select[name="spesialisasi"]');
            const filterButton = document.getElementById('filterButton');

            function checkFilters() {
                if (filterSpesialisasi.value !== "") {
            ...
            if (filterSpesialisasi) {
                filterSpesialisasi.addEventListener('change', checkFilters);
            }
