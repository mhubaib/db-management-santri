    ...
    <div class="bg-white overflow-hidden shadow-xl rounded-lg border border-gray-100">
        <div class="px-8 py-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-primary-800">Daftar Santri</h2>
            ...
            <div class="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-100">
                <form action="{{ route('santri.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                    ...
                    <div class="flex gap-3 flex-wrap sm:flex-nowrap">
                        ...
                        <select name="kamar"
                            ...
                            @foreach ($kamar as $k)
                                <option value="{{ $k->id }}" {{ request('kamar') == $k->id ? 'selected' : '' }}>
                                    {{ $k->nama_kamar }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        @if (request()->anyFilled(['search', 'kelas', 'kamar']))
                        ...
                        @forelse ($santris as $index => $santri)
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ ($santris->currentPage() - 1) * $santris->perPage() + $loop->iteration }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-primary-700">
                                    {{ $santri->nama }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ \Carbon\Carbon::parse($santri->tgl_lahir)->format('d M Y') }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 max-w-md">
                                    <span class="block truncate" title="{{ $santri->alamat_asal }}">
                                        {{ $santri->alamat_asal }}
                                    </span>
                                </td>
                            ...
                            <tr>
                                <td colspan="5"
                                    ...
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="font-medium">Tidak ada data santri</p>
                                        <p class="text-gray-400 mt-1">Silahkan tambahkan data santri terlebih dahulu</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                const filterKamar = document.querySelector('select[name="kamar"]');
                const filterKelas = document.querySelector('select[name="kelas"]');
                const filterButton = document.getElementById('filterButton');

                const allFilters = [filterKamar, filterKelas];
